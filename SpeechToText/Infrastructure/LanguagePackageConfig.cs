...
namespace Vmail._6.Infrastructure.MainModule
{
    public static class LanguagePackageConfig
    {
      ...
      public  static void Explore()
        ...
        static void RecognizeSpeechAndMakeSureTheComputerSpeaksToYou()
        {
            ...
            _recognizer = new SpeechRecognitionEngine();
            _recognizer.LoadGrammar(new Grammar(new GrammarBuilder("Kerelam"))); // load a "hello computer" grammar
        ...
        public static string RecognizeSpeechAndMakeSureTheComputerSpeaksToYou(string InputText)
        {

            _recognizer = new SpeechRecognitionEngine();
            _recognizer.LoadGrammar(new Grammar(new GrammarBuilder(InputText))); // load a "hello computer" grammar
            _recognizer.SpeechRecognized += _recognizeSpeechAndMakeSureTheComputerSpeaksToYou_SpeechRecognized; // if speech is recognized, call the specified method
            _recognizer.SpeechRecognitionRejected += _recognizeSpeechAndMakeSureTheComputerSpeaksToYou_SpeechRecognitionRejected;
            _recognizer.SetInputToDefaultAudioDevice(); // set the input to the default audio device
            _recognizer.RecognizeAsync(RecognizeMode.Multiple); // recognize speech asynchronous
        ...
        {
            if (e.Result.Text == "Kerelam")
            {
                SpeechSynthesizer speechSynthesizer = new SpeechSynthesizer();
                speechSynthesizer.Speak("you told Kerelam");
